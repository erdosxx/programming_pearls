#ifndef ALGORITHM_ANALYSIS_REDBLACKTREE_H
#define ALGORITHM_ANALYSIS_REDBLACKTREE_H
...
public:
    ...
    bool contains( const Comparable & x ) const {
        // nullNode is a sentinel. We can find it in the leaf.
        nullNode->element = x;
    ...
    // TODO: add remove function. See Ex 12.5 p626
...
private:
     ...
     */
    // See Fig 12.10 p586  adn Fig 12.11 p587
    void handleReorient( const Comparable & item ) {
        ...
        if( parent->color == RED ) {  // Have to rotate
            ...
            if( item < grand->element != item < parent->element )
                // item moved to parent location by rotation.
     ...
     */
    RedBlackNode * rotate( const Comparable & item, RedBlackNode *theGrandParent ) {
        if( item < theGrandParent->element ) {
            item < theGrandParent->left->element ?
            rotateWithLeftChild( theGrandParent->left )  :  // LL
            rotateWithRightChild( theGrandParent->left ) ;  // LR
            return theGrandParent->left;
        } else {
            item < theGrandParent->right->element ?
            rotateWithLeftChild( theGrandParent->right ) :  // RL
            rotateWithRightChild( theGrandParent->right );  // RR
            return theGrandParent->right;
        }
    }
...
#endif //ALGORITHM_ANALYSIS_REDBLACKTREE_H
