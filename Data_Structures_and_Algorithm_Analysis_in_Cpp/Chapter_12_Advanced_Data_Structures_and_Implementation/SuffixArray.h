#ifndef PROGRAMMING_PEARLS_SUFFIXARRAY_H
#define PROGRAMMING_PEARLS_SUFFIXARRAY_H
 ...
 * LCP is an existing array to place the LCP information
 */
void createSuffixArraySlow( const string& str, vector<int>* SA_ptr, vector<int>* LCP_ptr ) {
    vector<int>& SA = *SA_ptr;
    vector<int>& LCP = *LCP_ptr;

    if( SA.size() != str.size() || LCP.size() != str.size() )
        throw invalid_argument{ "Mismatched vector sizes" };

    vector<const char*> suffixes(str.size());
...
vector<pair<int, int>> createSuffixArray_erdos (const string& str) {
    enum {index= 0, LCP = 1};
    vector<pair<int, int>> result(str.size());

    for (int i = 0; i < result.size(); ++i) {
        get<index>(result[i]) = i;
    }

    sort( result.begin(), result.end(),
          [&, str] (const pair<int,int>& p1, const pair<int,int>& p2) {
            return str.substr(get<index>(p1)) < str.substr(get<index>(p2));
          }
    );
...
#endif //PROGRAMMING_PEARLS_SUFFIXARRAY_H
